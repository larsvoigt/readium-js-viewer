define(['hgn!templates/settings-dialog.html', 'ReaderSettingsDialog_Keyboard', 'i18n/Strings', 'Dialogs', 'storage/Settings', 'Keyboard', 'colpick', 'bootstrap'], function(SettingsDialog, KeyboardSettings, Strings, Dialogs, Settings, Keyboard){
    ...
    var initCustomThemeDialog = function(){
        ...
        $("#custom-theme-dialog").on('hidden.bs.modal', function(event) {
            $('#settings-dialog').modal('show');
            
        });

        $("#custom-theme-dialog").on('show.bs.modal', function(event) {
            
            var customThemeBtnForecolor = $("#custom-theme-btn").css("color");
            var customThemeBtnBackcolor = $("#custom-theme-btn").css("background-color");

            //$("#foreground-color-btn").colpickSetColor(customThemeBtnForecolor);
            //$("#background-color-btn").colpickSetColor(customThemeBtnBackcolor);
